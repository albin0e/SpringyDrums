...
MyModule::MyModule() {
    ...
    wireMasses.resize(numWires, 0.0001f);
...
void MyModule::reset() {
    ...
    radius = size * 2.54f / 200.f;  // Convert inches to meters
...
void MyModule::strikeDrum(float velocity) {
    ...
    float strikeForce = std::min(1.0f, velocity) * 0.01f;
    ...
    // Also reset snare components for a fresh hit
...
std::pair<float, float> MyModule::updatePhysics() {
    ...
    // Update states
    // Input scaling to not exceed +- 1 add here and try: added the next following lines
    float max_input = 1.0f;
    u_next = std::max(-max_input, std::min(u_next, max_input));
    u_prev = u;
    u = u_next;
    ...
    if (snareBlend > 0.f) {
        ...
        float coupling_factor = 0.3f * snareBlend;
        ...
        for (int i = 0; i < gridSize; i++) {
            for (int j = 0; j < gridSize; j++) {
                ...
                // Couple with main head (simplified physics)
        ...
        // Wire physics - calculate wire responses
        ...
        for (int w = 0; w < numWires; w++) {
            ...
            if (wire_pos >= 0 && wire_pos < gridSize) {
                ...
                // Calculate force on wire (only when head moves in the positive direction)
                float head_velocity = (snare_u_next[wire_pos][head_pos] - snareResonantHead[wire_pos][head_pos]) / dt;
                
                // Only apply force when velocity is positive
                float contact_force = 0.f;
                if (head_velocity > 0.f) {
                    contact_force = head_velocity * tension_factor;
                }
                ...
                // Check for wire hitting the head
                if (wireDisplacements[w] > 0.f) {
                    wire_response += wireDisplacements[w];
                    
                    // Wire bounces off with some energy loss
                    wireVelocities[w] = -wireVelocities[w] * 0.7f;
                    wireDisplacements[w] = 0.f;
                }
            }
        }
        ...
        // Generate real-time noise component
        ...
        // Apply simple bandpass filtering based on SnareTone parameter
        float snareTone = params[SNARE_TONE_PARAM].getValue();
        
        // 1. Apply first-order low pass filter
        float cutoffLow = 0.15f + snareTone * 0.45f;
        snareFilterStates[0] = snareFilterStates[0] +
            cutoffLow * (noise_sample - snareFilterStates[0]);
        
        // 2. Apply first-order high pass filter
        float cutoffHigh = 0.05f + snareTone * 0.5f;
        snareFilterStates[1] = cutoffHigh *
            (snareFilterStates[1] + noise_sample - snareFilterStates[2]);
        snareFilterStates[2] = noise_sample;
        
        // 3. Blend the two filtered signals
        float filtered_noise = snareFilterStates[0] + snareFilterStates[1];
        
        // Apply resonance based on SnareTone
        float resonance = 0.55f + snareTone * 0.4f; //0.6 before
        ...
        // Blend physical wire response and noise based on tone setting
        float noise_mix = 0.3f + (snareTone * 0.7f);
        float physical_mix = 1.0f - noise_mix;
        
        // Combine for final snare sound
        snare_output = (physical_mix * wire_response) + (noise_mix * noise_component);
        
        // Scale down snare output for better overall mix
        snare_output *= 0.15f; // 0.2 f before
    }
